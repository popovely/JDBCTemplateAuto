...
public class ListCommand implements BbsCommandImpl {
	...
	public ListCommand() {	// 확인용
		System.out.println("ListCommand 빈 자동생성");
	}
...
//		JDBCTemplateDAO dao = new JDBCTemplateDAO();
		...
		int pageSize = Integer.parseInt(EnvFileReader.getValue("SrpingBbsInit.properties", "springBoard.pageSize"));
		int blockPage = Integer.parseInt(EnvFileReader.getValue("SrpingBbsInit.properties", "springBoard.blockPage"));
		// 전체 페이지 수를 계산
		int totalPage = (int)Math.ceil((double)totalRecordCount / pageSize );
		// 현재 페이지번호. 첫 진입일 때는 무조건 1페이지로 지정
		int nowPage =
		...
		/** 페이징 추가 코드 end *************************************/
/**
		// 실제 출력할 리스트 가져오기 : 게시물을 select한 후 반환받음 (페이징 X)
...
*/
		...
		// JdbcTemplate을 사용할 때는 자원반납을 하지 않는다.
		// dao.close();
	}
}
