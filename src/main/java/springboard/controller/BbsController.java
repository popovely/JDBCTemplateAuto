...
public class BbsController {
	...
	// 모든 Command객체(CRUD)의 부모타입의 참조변수 생성
	...
	public String list(Model model, HttpServletRequest req, SpringBbsDTO springBbsDTO) {
		/**
		...
		model.addAttribute("springBbsDTO", springBbsDTO);	// springBbsDTO를 Model에 저장
//		command = new ListCommand();	// service역할의 ListCommand객체 생성
	...
	public String writeAction(Model model, HttpServletRequest req, SpringBbsDTO springBbsDTO) {
		/*
			...
			한번에 받을수 있다.
			Spring에서는 커맨드객체를 통해 이와같은 처리를 할 수 있다.
		 */
		// request객체와 함께 Model에 저장
		model.addAttribute("req", req);
		model.addAttribute("springBbsDTO", springBbsDTO);
		// 요청을 전달할 service객체를 생성한 후 execute()메소드 호출
...
//		command = new EditActionCommand();
		...
		/*
			수정처리가 완료되면 상세페이지로 이동하는데
			이 때 idx와 같은 파라미터가 필요하다.
			Model객체에 저장한 후 redirect하면 Spring에서는 자동으로 쿼리스트링 형태로 만들어준다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
