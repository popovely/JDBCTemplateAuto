...
public class JDBCTemplateDAO {
	...
	public ArrayList<SpringBbsDTO> list(Map<String, Object> map) {
		...
		/*
			...
			List컬렉션에 적재하여 반환한다.
			모델1, 모델2 방식의 DAO에서 개발자가 반복적으로 하던 작업을 자동으로 처리해 준다.
		 */
	...
	public void updateHit(final String idx) {
		...
		String sql = "UPDATE springboard SET "
				...
				+ " WHERE idx=? ";
		/*
			행의 변화를 주는 쿼리문 실행이므로 update메소드를 사용한다.
				첫번째 인자는 쿼리문,
				두번째 인자는 익명클래스를 통해 인파라미터를 설정한다.
		
		 - 92라인의 PreparedStatementCreator()와 기능은 같고,
		   다른점은 오버라이딩 받아야하는 메소드
		 */
	...
	public SpringBbsDTO view(String idx) {
		...
		try {
			/*
				...
				반드시 하나의 결과를 반환해야 한다.
				그렇지 않으면 에러가 발생하므로 예외처리를 하는것이 좋다.
			 */
			...
			/*
				...
				이 때 테이블의 컬럼명과 DTO의 멤버변수명은 일치해야 한다.
			 */
		}
		catch (Exception e) {
	...
	public int password(String idx, String pass) {
		...
		try {
			/*
				...
				queryForObject() 메소드는 반드시 하나의 결과가 나와야하고,
				그렇지 못한 경우에는 예외를 발생시키기 때문이다.
			 */
			SpringBbsDTO dto = template.queryForObject(sql,
					new BeanPropertyRowMapper<SpringBbsDTO>(SpringBbsDTO.class));
			/*
				일련번호는 시퀀스를 사용하므로 반드시 1이상의 값을 가지게 된다.
				따라서 retNum의 값이 0이 반환된다면 패스워드 검증 실패로 판단할 수 있다.
			 */
			retNum = dto.getIdx();
		}
		catch(Exception e) {
